package com.example.delibuddy.presentation.participationlist
...
class ParticipationListViewModel : ViewModel() {
    ...
    fun getRoomDeals() {
        viewModelScope.launch {
            try {
                val response = RetrofitClient.apiService.getRoomDeals("Bearer $USER_TOKEN")
    ...
    fun updateRoom(roomId: String) {
        ...
        viewModelScope.launch {
            try {
                val response = RetrofitClient.apiService.updateRoom("Bearer $USER_TOKEN", updateRoom)
                if (response.isSuccessful) {
                    getRoomDeals()
                    Log.d("testt", response.body().toString())
                } else {
