...
class RoomDetailViewModel : ViewModel() {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    response.body()?.let {
                        ...
                        if (_members.value.isNotEmpty() && _members.value.first().userId == USER_ID)
    ...
    fun joinRoom(roomId: String, userCost: Int) {
        ...
        viewModelScope.launch {
            try {
                val response = RetrofitClient.apiService.joinRoom("Bearer $USER_TOKEN", joinRoom)
    ...
    fun blockUser(userId: String) {
        ...
        viewModelScope.launch {
            try {
                val response = RetrofitClient.apiService.blockUser("Bearer $USER_TOKEN", userId)
